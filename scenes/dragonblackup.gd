...
var projectile_scene = preload("res://scenes/projectile_dragonup.tscn")
func _ready():
	ShootTimer.start()
...
func shoot(_target_position: Vector2):
	...
	var projectile_dragonup = projectile_scene.instantiate()
	
	# 2. Position it at the Muzzle
	projectile_dragonup.global_position = $Muzzle.global_position
	...
	get_tree().root.add_child(projectile_dragonup)
